...
function inviteBaseContent(sender: string, invitee?: { firstName: string }) {
  ...
  <p>Hello${
		invitee?.firstName ? `, ${invitee.firstName}` : ''
	}! this is an invitation from ${sender} to join his trip.</p>`;
}
...
export const inviteExistingUser = (
	...
	return {
		...
		html: `${baseContent}
      <p>To proceed, you can <a href="https://road-trip-client.vercel.app/trips/invitations">view your trip invitations</a> in our platform.</p>`
	};
};
...
export function inviteNewUser(sender: string, key: string) {
	...
	return {
		...
		html: `
				${baseContent}
				<p>To proceed, you can <a href=https://road-trip-client.vercel.app/signup?key=${key}>create an account</a> in our platform.</p>`
	};
}
